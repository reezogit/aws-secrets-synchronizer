    ...
    def get_encoded_data_to_sync(data, sync_empty):
        ...
        for key, value in data.items():
            if value is None and sync_empty is False:
                ...
                logging.warning("Secret %s has an empty value.", key)
            else:
...
def main():
    ...
    secret_syncer = SecretSyncer(
        ...
        logging.INFO,
        {
            'sync_interval': int(os.environ.get('SYNC_INTERVAL')) if 'SYNC_INTERVAL' in os.environ else None,
            'sync_empty': os.environ.get('SYNC_EMPTY'),
